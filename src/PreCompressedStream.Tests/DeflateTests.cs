...
{
    public class DeflateTests : TestBase
    {
        [Theory]
        [InlineData("")]
        [InlineData("a")]
        [InlineData("I am a compressed chunk of text")]
        public void Deflate_Roundtrip(string str)
        {
        ...
        [InlineData("a")]
        [InlineData("I am a compressed chunk of text")]
        public void Deflate_Precompressed_Uncompressed_Roundtrip(string str)
        {
            var data = PreDeflate(s => s.Write(Bytes(str)));
        ...
        [InlineData("a")]
        [InlineData("I am a compressed chunk of text")]
        public void Deflate_Precompressed_Combine_Compression_Levels(string str)
        {
            var data = PreDeflate(s =>
            {
        ...
        public void Deflate_Precompressed_Roundtrip(int times, CompressionLevel level)
        {
            var data = Bytes("I am a compressed chunk of text");
            var compressedData = Precompress(data, level);

            var returnedData = PreDeflate(s =>
            {
            ...
            var uncompressedData = DeflateDecompress(returnedData);
        ...
        [InlineData(CompressionLevel.Fastest)]
        [InlineData(CompressionLevel.NoCompression)]
        public void Deflate_Precompressed_Roundtrip_Different(CompressionLevel startLevel)
        {
            var data = Bytes("I am a compressed chunk of text");
